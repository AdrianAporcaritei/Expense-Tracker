...
function init() {
   ...
   let localState = JSON.parse(localStorage.getItem('ExpTracker'));
...
function onDeleteClick(event) {
    ...
    let deleteIndex;
    for (let i = 0; i < state.transactions.length -1; i++) {
...
function updateState() {
    ...
    localStorage.setItem('ExpTracker', JSON.stringify(state))

    render();
}

function render() {
    balanceEl.innerHTML = `${state.balance} lei`;
    incomeEl.innerHTML = `${state.income} lei`;
    expenseEl.innerHTML = `${state.expense} lei`;
    ...
    for (var i = 0; i < state.transactions.length; i++) {
        ...
        amountEl.innerHTML = `${item.amount} lei`;
